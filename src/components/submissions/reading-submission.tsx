  ...
  return (
    <div className="rounded-xl border border-gray-200 overflow-hidden">
      <div className="bg-indigo-50 px-5 py-3 border-b border-indigo-100">
        <h3 className="font-semibold text-indigo-800 text-sm">{task.title}</h3>
      </div>
          ...
          <div key={q.id} className="pt-3 first:pt-0">
                ...
                {(["TRUE", "FALSE", "NOT GIVEN"] as const).map((opt) => (
                  <button key={opt} type="button"
                    ...
                    className={`rounded-lg px-4 py-2 text-xs font-semibold border-2 transition ${
                      answers[q.id] === opt
                        ? opt === "TRUE" ? "border-green-500 bg-green-100 text-green-700"
                          : opt === "FALSE" ? "border-red-500 bg-red-100 text-red-700"
              ...
              <div className="space-y-1.5">
                {(q.options as string[]).map((opt, oi) => (
                  <label key={oi} className={`flex items-center gap-3 rounded-lg border-2 px-4 py-2.5 cursor-pointer transition ${
                    answers[q.id] === opt ? "border-indigo-500 bg-indigo-50" : "border-gray-100 hover:border-gray-200"
                  }`}>
                    <input type="radio" name={`q-${q.id}`} value={opt}
                      checked={answers[q.id] === opt}
                      onChange={() => onChange(q.id, opt)} className="accent-indigo-600" />
            ...
            {(task.type === "fill" || task.type === "short") && (
              <input
                ...
                placeholder={task.type === "fill" ? "Bo'sh joyga yozing..." : "Qisqa javob..."}
                value={answers[q.id] ?? ""}
...
export function ReadingSubmission({ content, submitted, onSubmit }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Passage */}
      <div className="rounded-xl border border-gray-200 overflow-hidden">
        <button type="button" onClick={() => setPassageOpen((o) => !o)}
          className="flex w-full items-center justify-between bg-blue-50 px-5 py-3 text-left hover:bg-blue-100 transition">
          <span className="flex items-center gap-2 text-sm font-semibold text-blue-800">
            <BookOpen className="h-4 w-4" /> Matn (Passage)
          </span>
          <span className="text-blue-500 text-xs">{passageOpen ? "Yopish ▲" : "Ochish ▼"}</span>
        </button>
        {passageOpen && (
          <div className="p-5 text-sm leading-relaxed text-gray-700 whitespace-pre-wrap max-h-80 overflow-y-auto">
            {content.passage}
          </div>
        )}
      </div>
